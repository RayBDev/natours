  ...
  switch (props.inputType) {
    case 'input':
      ...
      if (
        ...
        invalidMessage = (
          <span className="invalidMessage">{props.invalidMessage}</span>
        )
      } else {
        invalidMessage =
          !props.valueIsValid && props.touched ? (
            <div className="invalidMessage">{props.invalidMessage}</div>
          ) : null
      }
    ...
    case 'select':
      ...
      inputElement = (
        <select name={props.label} value={props.value} onChange={props.changed}>
          {optionList}
        </select>
      )
